...
use crate::state::effect::{ActiveEffect, EffectTarget};
    ...
    match &state.combat {
        Some(combat_state) => {
            ...
            let monsters: Vec<serde_json::Value> = combat_state.monsters.iter().enumerate().map(|(i, m)| {
                let monster_effects = state.effects.for_monster(i);
                ...
                if !monster_effects.is_empty() {
                    val["effects"] = serde_json::json!(
                        monster_effects.into_iter().map(effect_to_json).collect::<Vec<_>>()
                    );
                }
                val
            }).collect();
            ...
            let party_effects: Vec<serde_json::Value> = state.party.members.iter().filter_map(|c| {
                let char_effects = state.effects.for_character(&c.name);
                if char_effects.is_empty() {
                    None
                } else {
                    Some(serde_json::json!({
                        "character": c.name,
                        "effects": char_effects.into_iter().map(effect_to_json).collect::<Vec<_>>(),
                    }))
                }
            }).collect();

            // Build area/global effects
            let global = state.effects.global_effects();
            let global_effects: Vec<serde_json::Value> = global.iter().copied().map(effect_to_json).collect();

            // Append effects to status message
            let has_any_effects = !party_effects.is_empty()
                || combat_state.monsters.iter().enumerate().any(|(i, _)| !state.effects.for_monster(i).is_empty())
                || !global.is_empty();
            if has_any_effects {
                status.push_str("\n-- Effects --");
                for c in &state.party.members {
                    let effs = state.effects.for_character(&c.name);
                    if !effs.is_empty() {
                        let summaries: Vec<String> = effs.iter().map(|e| e.summary_line()).collect();
                        status.push_str(&format!("\n  {}: {}", c.name, summaries.join(", ")));
                    }
                }
                for (i, m) in combat_state.monsters.iter().enumerate() {
                    let effs = state.effects.for_monster(i);
                    if !effs.is_empty() {
...
pub(super) fn query_exploration(id: &str, state: &GameState) -> GMResponse {
    ...
    let active: Vec<String> = state.effects.effects.iter().map(|e| {
        let target_name = match &e.target {
            EffectTarget::Character(name) => format!(" on {}", name),
            EffectTarget::Monster(idx) => format!(" on monster #{}", idx),
            EffectTarget::Global => String::new(),
        };
        format!("{}{} ({})", e.name, target_name, e.duration)
    }).collect();
    if !active.is_empty() {
        status.push_str(&format!("\nEffects: {}", active.join(", ")));
    }
    ...
    if !state.effects.is_empty() {
        data["effects"] = serde_json::json!(
            state.effects.effects.iter().map(effect_to_json).collect::<Vec<_>>()
        );
    }
...
fn query_party_member_data(member: &party::results::PartyMemberSummary, effects: Vec<&ActiveEffect>) -> QueryPartyMemberData {
    ...
    match party::action_query_party(state) {
        Ok(result) => {
            let members: Vec<QueryPartyMemberData> = result
                ...
                .iter()
                .map(|m| {
                    let effects = state.effects.for_character(&m.name);
            ...
            for m in &result.members {
                let effs = state.effects.for_character(&m.name);
                if !effs.is_empty() {
