...
use crate::model::{Character, Monster};

use crate::bridge::registry::{character_index, is_character, is_monster, monster_index};
...
impl StateProvider for BridgeState {
    ...
    fn all_entities(&self) -> Vec<EntityRef> {
        let mut refs: Vec<EntityRef> = (0..self.characters.len())
            .map(|i| crate::bridge::registry::character_ref(i))
            .collect();
        refs.extend(
            (0..self.monsters.len()).map(|i| crate::bridge::registry::monster_ref(i)),
        );
        refs
    }
}
